...
public class Student extends BasicStudent {
    ...
    public JsonObject toJsonObject() {
        ...
        for (Tuple<String, Integer> exam: tests){
            ...
            JsonPair coursePassed = new JsonPair("passed", new JsonBoolean(passed));
            JsonObject jsTestObj = new JsonObject(courseName, courseMark, coursePassed);

            dataArr.add(jsTestObj);

        }

        jsonSt.add(new JsonPair("exams", new JsonArray(dataArr.toArray(new JsonObject[0]))));
        return jsonSt;
    }
}